...
function showSystemDetails(event) {
    ...
    if (systemHash) {
        ...
        sysDetails.innerHTML += "<li>Name: " + systemHash[event.target.id].name + "<br/></li>";
        sysDetails.innerHTML += "<li>Population: " + systemHash[event.target.id].population + " billion<br/></li>";
        sysDetails.innerHTML += "<li>Government: " + systemHash[event.target.id].govtype + "<br/></li>";
        sysDetails.innerHTML += "<li>Economy: " + systemHash[event.target.id].economy + "<br/></li>";
        sysDetails.innerHTML += "<li>Tech Level: " + systemHash[event.target.id].techlev + "<br/></li>";
        sysDetails.innerHTML += "</ul><br/>";
    }

    sysDetails.innerHTML += "<div style='border-top: 1px solid lime; padding-top:1em'>";
    ...
    for (var i = 0; i < commodities.length; i++) {
        ...
        if ((commodities[i] != null) || (commodities[i] != undefined)) {
            sysDetails.innerHTML += "-" + commodities[i].name + "    qty:" + commodities[i].quantity;
        }
        sysDetails.innerHTML += "</div>";
    }
    sysDetails.innerHTML += "</div>";

    drawCircleOutline(systemHash[event.target.id].x * 2, systemHash[event.target.id].y * 2, "red", 20)
}
    ...
    request.onload = function() {
            ...
            if (request.status >= 200 && request.status < 400) {
                solarSystems.forEach(system => {
                    ...
                    systemList.innerHTML += "<div id='" + system.name + "' onclick='showSystemDetails(event)' style='border-top: 1px solid #e9ce7c;padding:4px;cursor:pointer'>" + system.name + "</div>";
                    if (system.name)
                        systemHash[system.name] = system;
                    else
                        systemHash["commodities"] = system;
